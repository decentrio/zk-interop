...
contract Groth16Verifier {
    ...
    function verifyProof(
        ...
        assembly {
            ...
            checkField(calldataload(add(publicSignals, 0)))

            checkField(calldataload(add(publicSignals, 32)))

            checkField(calldataload(add(publicSignals, 64)))

            checkField(calldataload(add(publicSignals, 96)))

            checkField(calldataload(add(publicSignals, 128)))

            checkField(calldataload(add(publicSignals, 160)))

            checkField(calldataload(add(publicSignals, 192)))

            checkField(calldataload(add(publicSignals, 224)))

            checkField(calldataload(add(publicSignals, 256)))

            checkField(calldataload(add(publicSignals, 288)))

            checkField(calldataload(add(publicSignals, 320)))

            checkField(calldataload(add(publicSignals, 352)))

            // Validate all evaluations
            let isValid := checkPairing(proofA, proofB, proofC, publicSignals, pMem)
